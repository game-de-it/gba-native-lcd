...
現行リリースは汎用Android表示プロファイルではなく、**KONKR Pocket ADVANCE /
GT78-VN向けプロファイル**です。APK自体に機種名による起動制限はなく、Arm64
Android、OpenGL ES、加速度センサー、互換性のある横持ち操作環境を備えた別端末で
動作する可能性があります。ただし正確な再現には、960 x 640パネル、GBAの
240 x 160映像を正確に4倍表示できること、近いパネル色特性、同じセンサー軸が
必要です。
...
したがって、KPAは確認済み、その他のArm64 Androidハンドヘルドは実験的対応です。
別機種では実機確認後に専用プロファイルを用意するのが適切です。
...
AndroidでAPKを上書き更新するには、以前と同じ署名鍵が必要です。現在のローカル
ビルドは開発用鍵で署名されています。一般公開版では、以後の全バージョンで同じ
永続的なリリース鍵を使用する必要があります。異なる鍵で署名されたAPKを導入する
場合は既存パッケージのアンインストールが必要になり、前述のパッケージ固有設定が
削除される可能性があります。

リリース準備の進捗と、署名・ライセンスに関する残作業は
