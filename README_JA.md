...
シェーダーだけが必要な場合は、同じリリースにある
`GBA-Native-LCD-Shader-v0.1.0.zip`をダウンロードするか、
[`variants/`ディレクトリ](https://github.com/game-de-it/gba-native-lcd/tree/main/variants)を
参照してください。別端末で使用する前に、同梱の互換性に関する説明を確認してください。
...
エフェクト対応経路では、元の高速経路より大きなミキサーバッファが報告されます。
AudioFlinger上で確認したバッファの換算時間は約42 ms相当から約82 ms相当へ
増えました。これは実測したエンドツーエンドの出力遅延ではなく、すべての音が常に
82 ms遅れることを意味しません。EQのON・OFFいずれでも、通常プレイ中に体感できる
遅延差は確認されませんでした。
